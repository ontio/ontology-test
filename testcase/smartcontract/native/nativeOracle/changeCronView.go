...
func ChangeCronView(ctx *testframework.TestFrameworkContext, tx string) bool {
	wallet, err := ctx.Ont.OpenWallet("./wallet.dat", "passwordtest")
	if err != nil {
		ctx.LogError("OpenWallet ./wallet.dat error:%s", err)
		return false
	}
	...
	ctx.LogInfo("changeCronView result is : %+v", states)
